                ...
                Button(
                    onClick = {
                        ...
                        if(login.senha != "" && login.email != ""){
                            RetrofitFactory().getClienteService().loginCLient(login).enqueue(object : Callback<LoginResult> {
                                override fun onResponse(p0: Call<LoginResult>, res: Response<LoginResult>) {
                                    Log.i("Response:", res.body().toString())
                                    val clientJson = res.body()
                                        ?.let { Json.encodeToString(it.cliente) }
                                    if(res.isSuccessful){
                                        navController.navigate("home/${clientJson}")
                                        Log.i("Sucesso:", "Logou")
                                    }else{
